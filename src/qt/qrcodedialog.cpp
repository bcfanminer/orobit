...
{
    QString ret = QString("orobit:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ORO unit input in lnReqAmount, we generate the URI with ORO as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ORO, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
