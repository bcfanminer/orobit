...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uORO
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1390255654, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0xe025f5fe << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0xe0, 0x25, 0xf5, 0xfe };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OrobitMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OrobitMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("OrobitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("orobit-miner");
    ...
    try { loop {
        ...
        printf("Running OrobitMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
